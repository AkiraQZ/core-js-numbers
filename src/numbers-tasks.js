...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const valueX = (x2 - x1) ** 2;
  const valueY = (y2 - y1) ** 2;
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const vLength1 = Math.sqrt(x1 ** 2 + y1 ** 2);
  const vLength2 = Math.sqrt(x2 ** 2 + y2 ** 2);
  return Math.acos(scalarProduct / (vLength1 * vLength2));
}
 ...
 */
function isPrime(n) {
  // if n < 2 it's not primary
  if (n < 2) {
    return false;
  }
  // divide n on every number from 2 to n
  // if n divide on one of it, n not primary
  for (let i = 2; i < n; i += 1) {
    if (n % i === 0) {
      return false;
    }
